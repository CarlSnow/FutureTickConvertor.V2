...
namespace FutureTickConvertor
{
    ...
    public sealed class FutureTickFileNew 
    {
        ...
        /// 文件头长度
        /// </summary>
        const int C_FILEHEADER_LEN = 44;
        const int C_ORIGDAY_LEN = 8 * 4;
        const int C_TICK_OFFSET = C_FILEHEADER_LEN + C_ORIGDAY_LEN;

        /// <summary>
        /// 每条tick开始到盘口的字节数
        /// </summary>
        const int C_POS_TICKLEN = 32;
        ...
        public bool Read(List<Tick> ticklist, int offset, int count)
        {
            ...
            try
            {
                ...
                {
                    long pos = C_FILEHEADERLEN_OLD + offset * (C_TICKHEADERLEN_OLD + _quotecount * 2 * 8);
                    stream.Seek(pos, SeekOrigin.Begin);
        ...
        const int C_RESERVE_OLD = 27;
        /// <summary>
        /// 每条tick开始到盘口的字节数
        /// </summary>
        const int C_TICKHEADERLEN_OLD = 32;
        const int C_FILEHEADERLEN_OLD = 64;
        ...
        private void ReadOldHeader(BinaryReader reader)
        {
            ...
            reader.ReadBytes(C_RESERVE_OLD);
        }
        ...
        {
            int blklen = C_POS_TICKLEN + _quotecount * 2 * 8 - 4;
        ...
        private void UpdateTickOpenPrice(List<Tick> ticklist, BinaryWriter writer)
        {
            if (ticklist.Count > 0)
            {
                ...
                if (Math.Abs(openPrice - _openPrice) >= 0.0000001)
                {
                    ...
                    {
                        logger.Error("openPrice过大，出现异常" + openPrice);
                    }
                }
            }
        }
        ...
        public void FixTickOpenPrice(double lastPrice)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error("写入期货tick数据失败(" + _tickFileName + ")", ex);
            }
        }
        ...
        public bool Write(List<Tick> ticklist, int start, int len, int quotecount)
        {
            ...
            try
            {
                ...
                stream.Seek(C_FILEHEADER_LEN - 8, SeekOrigin.Begin);
        ...
        private void WriteHeader(BinaryWriter writer)
        {
            ...
            byte[] buffer = new byte[C_ORIGDAY_LEN + 8];
            writer.Write(buffer);
        }
        ...
        private void WriteOrigDays(BinaryWriter writer)
        {
            ...
            int itmp = (_origDays.Count << 12) + C_FILEHEADER_LEN;
            writer.Write((Int16)itmp);
            writer.Write((Int16)C_TICK_OFFSET);
            ...
            for (int i = offset; i < end; ++i)
            {
                ...
                if (tick.DateTime.Date != origday)
                {
                    if (_origDays.Count > 0)
                    {
                        ...
                        {
                            logger.Error("tick时序不对,tick日期=" + tick.DateTime.ToString("yyyyMMdd") + "文件日期=" + _origDays[_origDays.Count - 1].ToString("yyyyMMdd"));
                            continue;
                        }
        ...
        public bool Append(List<Tick> ticklist, int start, int len, int quotecount)
        {
            ...
            if (!finfo.Exists)
            {
                logger.Info("追加期货tick,文件不存在,覆盖(" + _tickFileName + ")");
                return Write(ticklist, start, len, quotecount);
            }
            
            if (finfo.Length < C_FILEHEADER_LEN + C_ORIGDAY_LEN)
            {
                logger.Info("追加期货tick,文件大小不对,覆盖(" + _tickFileName + ")");
                return Write(ticklist, start, len, quotecount);
            }

            if (((finfo.Length - C_TICK_OFFSET)%(C_POS_TICKLEN + quotecount*2*8)) != 0)
            {
                logger.Info("追加期货tick,文件格式不对,覆盖(" + _tickFileName + ")");
                return Write(ticklist, start, len, quotecount);
            }
            ...
            try
            {
                        ...
                        1024*1024))
                {
                    ...
                    var ftickcount = (int)((finfo.Length - C_TICK_OFFSET) / (C_POS_TICKLEN + quotecount * 2 * 8));
                    if (ftickcount != _tickcount)
                    {
                        logger.Error("文件大小(" + ftickcount + ")和tick数(" + _tickcount + ")不符,可能有错: 文件=" + _tickFileName);
                    }
            ...
            catch (Exception ex)
            {
                ...
                logger.Error("追加期货tick数据失败(" + _tickFileName + ")", ex);
            }
        ...
        public static List<DateTimeSlice> GetDateTimeSlice(string strFile)
        {
            ...
            try
            {
                ...
                if (tickfile.ReadHeader(reader))
                {
                    ...
                    int blklen = C_POS_TICKLEN + tickfile._quotecount * 2 * 8 - 4;
                ...
                else
                {
                    ...
                    int blklen = C_POS_TICKLEN + tickfile._quotecount * 2 * 8 - 4;
                    stream.Seek(C_FILEHEADERLEN_OLD, SeekOrigin.Begin);
